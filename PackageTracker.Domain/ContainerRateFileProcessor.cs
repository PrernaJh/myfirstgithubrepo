...
namespace PackageTracker.Domain.Processors
{
    public class ContainerRateFileProcessor : IContainerRateFileProcessor
    {
        private readonly IActiveGroupProcessor activeGroupProcessor;
        private readonly IRateRepository rateRepository;
        private readonly ILogger<ContainerRateFileProcessor> logger;

        public ContainerRateFileProcessor(IActiveGroupProcessor activeGroupProcessor,
            IRateRepository rateRepository,
            ILogger<ContainerRateFileProcessor> logger)
        {
        ...
        public async Task<FileImportResponse> ImportContainerRatesFileToDatabase(Stream fileStream, Site site)
        {
            ...
            try
            {
                ...
                var rates = await ReadContainerRatesFileStreamAsync(fileStream);
        ...
        private async Task<List<Rate>> ReadContainerRatesFileStreamAsync(Stream stream)
        {
            try
            {
                    ...
                    while (!reader.EndOfStream)
                    {
                        ...
                        if (passedFirstLine)
                        {
                            ...
                            if (parts.Length > 27 &&
                                ...
                                StringHelper.Exists(parts[2]) && parts[2] != RateFileConstants.Package)
                            {
                                rates.Add(new Rate
                                {
                                    Carrier = parts[0],
                                    Service = RateUtility.AssignServiceTypeMapping(parts[1], parts[2]),
