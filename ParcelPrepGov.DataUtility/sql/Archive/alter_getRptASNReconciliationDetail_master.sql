...
BEGIN
	...
	SELECT DISTINCT pd.PackageId AS PACKAGE_ID, pd.CosmosCreateDate AS IMPORT_DATE, pd.PackageStatus AS PACKAGE_STATUS, pd.SubClientName AS SUB_CLIENT_NAME
	INTO #TempScanned
	...
	WHERE pd.SubClientName = @subClientName
		...
		AND pd.ShippedDate IS NULL
		GROUP BY pd.SubClientName, pd.PackageId, pd.CosmosCreateDate, pd.PackageStatus
