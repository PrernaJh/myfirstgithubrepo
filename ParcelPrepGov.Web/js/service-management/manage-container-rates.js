...
PPG.ManageContainerRates = function ($) {
    ...
    let _files, _selectedActiveGroupId, _selectedStartDate, _downloadFileName, _siteName = null;
    ...
    const startDate_onInitialized = function (e) {
        ...
        _startDate = _siteLocalDate;
    }

    const setSiteLocalDate = function (e) {
        var ajax = PPG.ajaxer.get('/ServiceManagement/GetSiteLocalDate?siteName=' + _siteName);
    ...
    const submitButton_onClick = function (e) {
        ...
        formData.append('siteName', getSelectedSite());

        let ajax = PPG.ajaxer.postFormData('/servicemanagement/UploadContainerRates', formData)
    ...
    const grid_onSelectionChanged = function (e) {
        ...
        _downloadFileName = `rates_download_${_siteName}_${month}${day}${year}`;
    ...
    const exportButton_onClick = function (e) {
        _ajaxingGrid.start();
        var ajax = PPG.ajaxer.post('/ServiceManagement/GetContainerRatesByActiveGroupId', { id: _selectedActiveGroupId });
        ajax.done(function (response) {
...
PPG.manageContainerRates = new PPG.ManageContainerRates(jQuery);